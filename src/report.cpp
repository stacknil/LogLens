...
void remove_stale_report_output(const std::filesystem::path& path) {
    std::error_code error;
    std::filesystem::remove(path, error);
    if (error) {
        throw std::runtime_error(
            "unable to remove stale report output: " + path.string() + ": " + error.message());
    }
}
...
void write_reports(const ReportData& data, const std::filesystem::path& output_directory, bool emit_csv) {
    ...
    if (!emit_csv) {
        remove_stale_report_output(findings_csv_path);
        remove_stale_report_output(warnings_csv_path);
        return;
    }
