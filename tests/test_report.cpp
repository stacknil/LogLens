...
void test_write_reports_removes_stale_csv_when_csv_is_disabled() {
    const auto output_directory = std::filesystem::current_path() / "report_stale_csv_cleanup_test";
    std::filesystem::remove_all(output_directory);

    loglens::write_reports(make_report_data(), output_directory, true);
    expect(std::filesystem::exists(output_directory / "findings.csv"),
           "expected findings.csv after csv-enabled report run");
    expect(std::filesystem::exists(output_directory / "warnings.csv"),
           "expected warnings.csv after csv-enabled report run");

    loglens::write_reports(make_report_data(), output_directory, false);

    expect(!std::filesystem::exists(output_directory / "findings.csv"),
           "expected stale findings.csv to be removed when csv is disabled");
    expect(!std::filesystem::exists(output_directory / "warnings.csv"),
           "expected stale warnings.csv to be removed when csv is disabled");

    std::filesystem::remove_all(output_directory);
}

void test_write_reports_reports_stale_csv_cleanup_failure() {
    const auto output_directory = std::filesystem::current_path() / "report_stale_csv_failure_test";
    std::filesystem::remove_all(output_directory);
    ...
    expect(threw, "expected write_reports to fail when stale findings.csv cannot be removed");
    expect(message.find("findings.csv") != std::string::npos,
           "expected stale csv cleanup failure to mention findings.csv");
}
...
int main() {
    ...
    test_write_reports_reports_stale_csv_cleanup_failure();
    return 0;
}
